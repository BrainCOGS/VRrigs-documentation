...
# {{ $frontmatter.title }}
 ...
 + Several BRAINCoGS processes are triggered automatically.
 + All of these processes run on the **u19proc2** (g-bcogs-u19proc2.pni.princeton.edu) virtual machine, administered by PNI Help. Contact Garrett T. McGrath (gmcgrath@princeton.edu) for permissions.
 + All processes run under the **u19prod** user account, so they aren't linked to any personal account.
...
## Alert system daily routine
 ...
 + Executes **main_old_log_deletion()**: deletes old logs (> 2 weeks) from all cronjobs' main log directories.
...
## Behavior, Manipulation, Optogenetics, Pupillometry tables ingestion (MATLAB Cronjob)
 ...
 + **Overview:** Call <a href='https://github.com/BrainCOGS/U19-pipeline-matlab/blob/master/scripts/populate_tables.m '> populate_tables.m script</a>. Ingests all Behavior-related tables from that day's new acquisition.SessionStarted & acquisition.Session records, and ingests the schedule for the next day. Check the populate_tables.m script for more information.
...
## Tech Alert
 ...
 + Executes **cronjob_locked_tables_alert()**: checks for locked tables at startup (in theory, a locked table indicates a non-writable DB) and sends a dev-notifications alert.
 + Executes **cronjob_schedule_check()**: checks whether no subjects are scheduled for training the next day, which most likely means the schedule population script failed, and sends a dev-notifications alert.
...
## Missing Sync Behavior
 ...
 + **Overview:** Call <a href='https://github.com/BrainCOGS/U19-pipeline-python/blob/master/u19_pipeline/automatic_job/populate_missing_syncbehavior_ephys.py'> populate_missing_sync_behavior_ephys.py script </a>. Populates the **ephys_pipeline.BehaviorSync** table. Since BehaviorSync requires behavior data to already be present in the DB for the session (done <a href='https://braincogs.github.io/software/automated_cronjobs.html#behavior-manipulation-optogenetics-pupillometry-tables-ingestion-matlab-cronjob'> here </a> at 3 a.m), this table is populated outside the ephys pipeline, here instead.
...
## Live Stats Monitor routine
 ...
 + **Overview:** Call <a href='https://github.com/BrainCOGS/U19-pipeline-python/blob/master/u19_pipeline/alert_system/live_monitor_alert/live_monitor_alert.py#L67'> Main Live monitor Function </a>. Checks for subjects with a "long" time (> 20 min) since their last valid, successful training trial, and raises an alarm if any are found.
...
## Ephys/Imaging Automation Pipeline process
 ...
 + **Overview:** Call <a href='https://github.com/BrainCOGS/U19-pipeline_python/blob/masteru19_pipeline/automatic_job/cronjob_automatic_job.py'> cronjob_automatic_job.py (Ephys Imaging cronjob script) </a>. Checks whether a new recording has been added via <a href='https://github.com/BrainCOGS/RecordingProcessJobGUI '> RecordingProcessJobGUI </a> or a job (recording process) has advanced to a new status. If either has occurred, the "next" function or process is called for that recording and/or job.
